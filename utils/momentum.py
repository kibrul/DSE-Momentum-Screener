...
def adr_pct(df: pd.DataFrame, window: int = 20) -> float:
    ...
    valid_low = recent["Low"].where(recent["Low"] > 0)  # guard against 0/negative Low (bad data)
    daily_range_pct = (recent["High"] / valid_low - 1) * 100
    if daily_range_pct.dropna().empty:
        return float("nan")
...
def is_tight_consolidation(df: pd.DataFrame, window: int = 10, max_range_pct: float = 8.0) -> bool:
    ...
    if pd.isna(low_min) or pd.isna(high_max) or low_min <= 0:
        return False
    range_pct = (high_max / low_min - 1) * 100
